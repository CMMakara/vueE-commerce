<template>
   <header>
      ...
      <base-modal v-if="show" header="Confirm Logout" @close="show = false">
      <template #content>
         <p class="m-2">Are you sure you want to logout?</p>
      </template>
      <template #footer>
         <button class="btn btn-secondary button-cancel" @click="show = false">Cancel</button>
         <button class="btn btn-danger button-logout" @click="handleLogout">Logout</button>
      </template>
   </base-modal>
   </header>
...
let counted = computed(() => cartStore.items.length > 0 ? cartStore.items.length : false)
...
.button-cancel {
  ...
  cursor: pointer;
  padding: 8px;
  box-shadow: 0px 6px 0px #cfcfcfc6;
  transition: all 0.1s;
}
