...
let loading = ref(true);        // Initial skeleton loading
let loadingPage = ref(false);   // Infinite scroll loading (bottom)
let hasMore = ref(true);        // Stop if no more products
...
const toggleAll = () => { selectedCategories.value = [] }

// Filtered items based on store data
const filterProduct = computed(() => {
   const products = allProducts.allProducts;
   if (selectedCategories.value.length === 0) return products;
   return products.filter(product =>
      product.categories.some(cat => selectedCategories.value.includes(cat.name))
   )
})
...
async function loadNextPage() {
   ...
   const newData = await allProducts.getAllProduct({ 
      page: page.value, 
      per_page: per_page.value, 
      sortDir: sortDir.value 
   });

   // If the API returns fewer items than per_page, we've reached the end
   if (!newData || newData.length < per_page.value) {
      hasMore.value = false;
   }
...
onMounted(async () => {
   ...
   loading.value = true;
   await Promise.all([
      allProducts.getAllProduct({ page: 1, per_page: per_page.value, sortDir: sortDir.value }),
...
<template>
   ...
   <section class="mt-4">
      ...
      >
         <template #filterBtn>
            <button @click="isFilter" class="btn btn-dark py-3 pe-5 ps-2 rounded-4 fw-medium shadow-sm">
               <i class="bi bi-filter-left"></i>&nbsp;<span>Filter</span>
            </button>
         </template>

         <template #filter>
            <div v-if="filter" class="col-3 category-sidebar">
               <div class="rounded p-3 border bg-light sticky-top" style="top: 20px;">
                  ...
                  <div class="form-check mb-2" v-for="option in categoryStore.categoryName" :key="option.id">
                     <input class="form-check-input" type="checkbox" :id="'category' + option.id" :value="option.name" v-model="selectedCategories">
