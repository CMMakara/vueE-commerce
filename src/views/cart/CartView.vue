        ...
        <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
          <div v-for="item in cartStore.items" :key="item.id"
            ...
            <div class="d-flex align-items-center gap-3 flex-grow-1">
              ...
              <div>
                ...
                <div class="mt-2 d-flex align-items-center gap-2">
                  <button @click="decrement(item)" class="btn btn-outline-danger btn-sm rounded-1 p-0"
                    style="width: 32px; height: 32px">
                    −
                  </button>
                  <span class="px-3">{{ item.qty }}</span>
                  <button @click="increment(item)" class="btn btn-outline-primary btn-sm rounded-1 p-0"
                    style="width: 32px; height: 32px">
                    +
                  </button>
                </div>
            ...
            <div class="text-end">
              ...
              <button @click="confirmDelete(item.id)" class="btn btn-outline-danger btn-sm rounded-1 p-2 shadow-sm"
                title="Remove Item">
...
async function increment(item) {
  ...
  //   fmdata.append('product_id', item.id)
  //   fmdata.append('qty', item.qty)  

  //   await api.post('/api/carts', fmdata, {
...
async function deleteItem() {
  await api.delete(`/api/carts/${itemToDelete.value}`);
  closeModal();
  cartStore.fetchCart();
